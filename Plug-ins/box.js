	...
	var Fancyspinbox = function (origSelect, options) {
		...
		this.down = function () {
			...
			if (selected > 0) {
				...
				if (settings.cycle) selected = origSelect.options.length - 1;
				else return false;
			}
			updateSelected();
			return selected;
		...
		var init = (function () {
				...
				var $croppedList = $fancy.find(".crop ul").css({
					"margin-top": spinboxHeight * selected * -1
				});
				if (settings.scroll) $croppedList.css({
					"transition": "margin-top " + settings.scrollDuration + "ms ease"
				});

				// event handlers
				$fancy.find(".spin-up").on("mousedown", function (e) {
					e.preventDefault();
					if (!settings.flipSpinDirection) obj.down();
					else obj.up();
					if (settings.focusOnSpin) $fancy.focus();
				});
				$fancy.find(".spin-down").on("mousedown", function (e) {
					e.preventDefault();
					if (!settings.flipSpinDirection) obj.up();
					else obj.down();
					if (settings.focusOnSpin) $fancy.focus();
				});
			...
			updateSelected = function () {
				if (supportsTransitions || !settings.scroll) {
					$fancy.find(".crop ul").css({
						"margin-top": spinboxHeight * selected * -1
					});
				} else {
					$fancy.find(".crop ul").animate({
						"margin-top": spinboxHeight * selected * -1
